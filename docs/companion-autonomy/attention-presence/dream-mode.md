...
Dream Mode has four passes. The first three belong to the offline compiler. The
fourth activates the resulting knowledge in the live system.
...
Dream compiler
  ...
  -> extended behavior: triage and analysis
  -> advanced behavior: consolidation

Runtime activation
  -> activation behavior: feature-gated consumption of Dream artifacts
```
...
### 5.1 default behavior: Collection

default behavior establishes durable raw inputs for later Dream work.

Primary responsibilities:

- persist compact per-iteration logs from the DurableLoop
...
### 5.2 extended behavior: Triage and Analysis

extended behavior reads only unprocessed material and turns it into candidates.
...
### 5.3 advanced behavior: Consolidation

advanced behavior promotes repeated signals into durable knowledge and writes Soil-compatible updates.
...
This stage should be the main place where Dream becomes useful to Soil. The output is not a log dump; it is a typed update set that can be applied safely and repeatedly.

### 5.4 activation behavior: Activation

activation behavior is separate from the compiler itself. It lets runtime consumers use Dream outputs during normal execution.
...
In the current runtime, Dream activation features default to on because PulSeed
uses an experimentation-heavy posture and early users are expected to actively
exercise memory-driven behaviors. Operators who want a more conservative posture
can still disable individual activation flags in `~/.pulseed/dream/config.json`.
