...
| --- | --- | --- |
| Agentic friend | Product direction: companion software that can remember context, notice change, prepare help, and back off when silence is safer. | Not a claim that every companion workflow is current package behavior. Current behavior is listed in [Status](../operate/status.md). |
