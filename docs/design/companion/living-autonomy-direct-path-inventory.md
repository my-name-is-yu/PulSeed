...
> Status: Public implementation inventory for the non-GUI companion-autonomy foundation. The machine-readable inventory in source remains the runtime policy owner.

> Scope: Direct-path inventory for the non-GUI companion-autonomy foundation.
...
outward effects before the shared attention/delivery integrations. Internal,
...
as current debt above, that later runtime integrations must wire into this
contract. Until those integrations land, this file and the typed inventory are
the owner map and regression guard for finding bypasses.
