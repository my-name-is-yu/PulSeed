...
> Status: Contract slice for issue #1951. This document describes the typed
> projection introduced in `src/runtime/decision/`. It does not define a new
> memory store and does not replace Surface, Soil, KnowledgeManager, or the
> correction/quarantine substrate.
