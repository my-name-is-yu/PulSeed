...
> Status: Historical design audit retained for context. This page is not the
> current docs architecture; use [Design Index](../index.md) for the current
> public design corpus.

This page preserves the audit basis for the docs rearchitecture work as public
historical design context. Snapshot date: 2026-05-11. Package snapshot: v0.6.5.
Do not read the inventory below as current package status.
...
product design, design, or historical material.

## Snapshot Documentation Problems
...
- `docs/operate/status.md` was dated `2026-04-12`, while the release history was
...
## Snapshot Implemented Surfaces To Reflect

- Package snapshot: npm `pulseed`, version `0.6.5`, ESM, `pulseed` binary, Node
  `>=22 <23 || >=24 <25`.
...
- Historical context: earlier design drafts, issue-order notes, and proposal material retained for traceability.
...
This was the originally proposed work breakdown for the documentation
rearchitecture. It is retained as history, not as a live commit plan.

1. audit current docs information architecture
2. add documentation structure
3. rewrite README and first-reader path
4. align guides and references with current runtime
5. separate design, future, and historical material
6. tighten docs checks and polish links
