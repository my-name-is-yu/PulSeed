...
> Status: Public historical design context. This page explains earlier architecture decisions and migration context; current behavior is described by the linked active docs.

> Scope: Evidence audit for Companion Capability Runtime after the implementation sequence.

This is a dated implementation-evidence snapshot. It is retained to explain how
the capability-runtime contract was checked at that point in the project, not as
the current operator/status surface.

This audit maps the Companion Capability Runtime contract to code, tests,
and documentation evidence after the implementation checkpoint. It is not a new
contract; the source contract remains
...
- the implementation order used for that checkpoint
- the code and tests that landed the contract

The final checkpoint did not add feature runtime behavior. The only remaining
gaps found are follow-up integration work that should stay focused and
...
| Treat Dream procedural hints as planning hints, not skills/plugins/execution authority. | `src/platform/observation/capability-graph.ts`, docs in `docs/operate/configuration.md`, and capability graph tests. | Covered. Dream assets project planning-hint-only capability candidates. |
...
| Keep operator/status/debug surfaces truthful about readiness plus admission/autonomy. | `src/runtime/control/capability-status-projection.ts`, `src/interface/cli/commands/operator-binding-status.ts`, `docs/operate/status.md`, `docs/operate/runtime.md`, and `README.md`. | Covered for projection shape and printing. Follow-up work covers collecting live runtime projections into operator status. |
| Keep inbound access, outbound notification permission, reply targets, notification routes, runtime-control admission, and autonomous authority separate. | `src/runtime/gateway/channel-policy.ts`, `src/interface/chat/ingress-router.ts`, `src/interface/chat/cross-platform-session.ts`, `src/runtime/control/runtime-control-service.ts`, and gateway/chat tests. | Covered by gateway/chat caller-path tests. |
| Keep runtime event creation separate from user notification. | `src/runtime/control/__tests__/runtime-control-service.test.ts` and notification dispatcher/outbox tests. | Covered. Runtime control events do not dispatch notification without the notification dispatcher path. |
...
| Exercise production caller-path shapes where boundaries cross chat, gateway, runtime, permission, or external surfaces. | Caller-path tests in `chat-runner.test.ts`, `cross-platform-session.test.ts`, `runtime-control-service.test.ts`, `InteractiveAutomationTools.test.ts`, and `executor.test.ts`. | Covered for the current boundary set. |
...
capability-runtime spine because the underlying contracts, evidence stores,
projections, and caller-path regression tests already exist.

## Closing Assessment

The implementation checkpoint established concrete owner boundaries for asset
