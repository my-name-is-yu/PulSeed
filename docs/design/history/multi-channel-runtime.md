...
> Historical architecture draft. For the current production recovery design, see [Runtime Auto-Recovery](../runtime/runtime-auto-recovery.md).
...
The priority model directly mirrors PulSeed's drive model from [Drive System](../loop/drive-system.md). A goal with a high gap score generates HIGH-priority events; a passive monitoring goal with low drive generates NORMAL-priority events. Priority is not a hack; it is the queue-layer expression of the same urgency judgment that already exists in the system.
...
User-visible workspace artifacts, debug exports, reports, and configuration remain file-backed where they are not internal durable runtime state. The current database-first boundary is maintained in `docs/design/platform/database-first-state-ownership.md`.
