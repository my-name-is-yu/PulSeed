...
> Scope: Target contract for Codex-like interaction behavior.

This document defines the target interaction contract for PulSeed's
Codex-like natural-language chat surfaces. It is a public design contract, not a
claim that all behavior already exists. The golden fixture at `tests/fixtures/codex-like-interaction-contract.golden.json`
contains the same contract in a machine-checkable form for production tests.
