...
> Implementation alignment: chat and TUI now use the stronger native AgentLoop path. `/tend` is retained here as a handoff pattern from bounded chat execution into long-lived goal control.
