...
> Implementation alignment: these optimization principles now apply across DurableLoop, bounded AgentLoop phases, chat compaction, and native task execution rather than only a single flat loop body.
...
| Mode | Module | LLM Calls | Input Tokens (est.) |
|-------|--------|-----------|-------------------|
...
The observation stage dominates because it scales linearly with dimension count AND loop count. Task generation and verification are constant per iteration. This means observation is the highest-leverage optimization target.
...
Even after observation completes, the remaining loop stages (gap calculation, drive scoring, task generation, execution, verification) run unconditionally. When the goal state has not meaningfully changed, these stages produce identical outputs and waste both LLM tokens (task generation, verification) and wall-clock time.

### Design

After observation completes, compute a **state diff** against the previous iteration. If no meaningful change is detected across ALL dimensions, skip the remaining loop stages entirely.
...
- `src/orchestrator/loop/durable-loop.ts` -- add diff check after observation stage, before gap calculation
...
This is optional (extended behavior). The default static classification is sufficient for launch.
...
| Mode | Baseline calls | After Pillar 1 | After Pillar 2 | Calls remaining |
|-------|---------------|----------------|----------------|-----------------|
...
### Area A: 2-Model Routing (Pillar 3)
...
### Area B: Staged Observation (Pillar 1)
...
### Area C: State Diff + Loop Skip (Pillar 2)
...
2. Integrate into `durable-loop.ts` after observation advanced behavior. Add consecutive skip limit and interaction with stall detection
...
### Area D (optional): Tier Override Config
...
If the loop keeps skipping, stall detection in the later loop stage never executes.
...
## 8. Not Included / Extension Designs
