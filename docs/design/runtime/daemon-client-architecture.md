...
**Scope:** Daemon/client architecture reference
**Source:** Earlier public design draft
**Date:** 2026-04-05

> Implementation alignment: TUI, chat, daemon, and schedule flows now sit on the shared DurableLoop + AgentLoop runtime stack. This page records the daemon/client ownership direction.
...
1. EventServer SSE + REST endpoints
2. DaemonRunner always-on EventServer + event emission
3. `daemon-client.ts`
4. TUI entry daemon detection + auto-start
5. TUI `use-loop.ts` SSE-driven state
6. Integration coverage for the full flow
