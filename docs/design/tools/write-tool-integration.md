...
> Implementation alignment: ChatRunner is no longer the only tool-using path; the native AgentLoop runtime, DurableLoop phases, and task execution all share the built-in tool substrate. This page records the migration rationale for that shared substrate.
