...
> Implementation alignment: plugins sit beside built-in tools and native `agent_loop` execution. They are one capability surface in PulSeed's broader execution architecture.
...
PulSeed:
  DurableLoop → "Which source is optimal for observing this dimension?"
...
## §5 PulSeed Autonomous Plugin Selection Modes

PulSeed's plugin autonomy is described through three selection modes. These modes define authority boundaries, not a rollout sequence.

### Default behavior — Manual configuration
...
The default behavior has zero autonomy. "Which plugins to use" is entirely determined by the user. However, "when to call plugins" is decided by PulSeed within the DurableLoop.

### Extended behavior — Capability auto-matching
...
5. Suggest candidate plugins to user (run automatically but with notification)
   Or auto-select if trust_score meets the advanced-behavior criteria
```

`ObservationEngine.findDataSourceForDimension()` is extended in extended behavior to account for plugin capability matching.
...
async findDataSourceForDimension(dimensionName: string): Promise<IDataSourceAdapter | null> {
  ...
  // extended-behavior extension: search by plugin manifest's dimensions[]
...
### Advanced behavior — Trust-based learning selection
...
Newly added plugins start at trust_score = 0. Since they have not reached the advanced-behavior auto-selection threshold (+20), the user is always notified before first use. Once track record accumulates and the threshold is exceeded, autonomous selection occurs without notification.
...
### 7.1 Execution model (default behavior)

In the default behavior, plugins run in the same process as PulSeed. No sandbox is applied. The rationale and constraints for this decision are as follows:

| Item | Details |
|------|---------|
| Execution model | Same process (Node.js dynamic import) |
| Sandbox | None (default behavior) |
| Trust assumption | Only targets plugins manually installed by the user |
| Future | VM isolate or Worker Threads considered in extended behavior |
...
In the default behavior, permission declarations are **informational only** and are not enforced at runtime. They serve as prerequisite information for adding permission enforcement in future extensions. Plugins declaring `shell: true` display an explicit warning during `pulseed plugin install`.
...
| `ObservationEngine` | Add plugin matching to `findDataSourceForDimension()` | Via `DataSourceRegistry` (extended in extended behavior) |
...
| `CapabilityDetector` | Reference plugin manifests in `detectGoalCapabilityGap()` | Receives manifest list from `PluginLoader` (extended behavior) |
| `TrustManager` | Add plugin trust_score tracking | Updates `PluginState.trust_score` (advanced behavior) |
| `KnowledgeManager` | Record and share plugin effectiveness | Receives `onPluginSuccess/Failure()` hooks (advanced behavior) |
...
| Autonomy modes | Manual configuration, capability matching, and trust-based learning have distinct authority boundaries |
