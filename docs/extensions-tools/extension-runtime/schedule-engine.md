...
> Related: `plugin-architecture.md`, `reporting.md`, `daemon-client-architecture.md`, `docs/runtime-architecture/execution-loop/signal-and-drive/observation.md`, `docs/goal-execution/planning-context/context-sources/data-source.md`

> Implementation alignment: scheduling now coexists with the dual-loop architecture. A scheduled activation may ultimately drive DurableLoop, which in turn may invoke bounded AgentLoop phases and native task execution. Read "full DurableLoop" in this document as "long-lived control path," not as a single flat sequence without internal agentic phases.
...
### Default behavior: Core infrastructure + Heartbeat (lightest layer first)
...
### Extended behavior: Probe layer + escalation
...
### Advanced behavior: Cron + GoalTrigger layers
...
### Activation behavior: Plugin extension + migration
