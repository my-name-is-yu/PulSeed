...
This design proposes a fundamental shift: **tools are the universal capability layer that sits beneath every operation in the DurableLoop.** Not "execution capability bolted onto area C," but tools as infrastructure that makes observation faster, gap calculation more accurate, knowledge acquisition cheaper, and verification more reliable.
...
4. **The DurableLoop tightens.** Each iteration can complete faster because perception and verification are direct.
...
2. A DurableLoop phase tags the tool as relevant (e.g., verification phase loads `Shell`).
...
## 9. Tool Integration: Task Execution (Extension Designs)

> **Note**: This section describes extension designs that builds on the tool system established in Sections 3-8. It is NOT part of the current implementation scope. The concepts are documented here for architectural completeness and to inform the design of the read-only tool system.
...
## 13. Related Design Surface Alignment

The tool system affects several design surfaces. The public contract is:

- PulSeed orchestrates direct tools, command execution, adapters, and agents
  through explicit policy and permission boundaries.
- Read-only tools support mechanical observation: file checks, code search,
  configuration reads, and simple API health checks.
- Command tools are execution surfaces. They require command policy and
  permission checks before they can affect the local environment.
- Agent delegation remains the path for multi-step reasoning, broad research,
  human interaction, or work that exceeds the tool contract.
- The runtime boundary is not "PulSeed versus all external interaction." It is
  "typed read, command, and mutation tools plus agents and adapters behind
  policy."

Related docs should preserve that distinction:

- [Tool Substrate](./tool-substrate-map.md)
- [Extension Runtime](../extension-runtime/extension-runtime-map.md)
- Runtime operations docs for the current command surface.
...
**Decision** (resolved): Implement per-iteration cache keyed by `(toolName, JSON.stringify(input))`. Cache expires at end of each DurableLoop iteration. All read-only tools are cacheable. ShellTool results cacheable only for safe-list commands. This is a PulSeed differentiation — CC does NOT cache tool results (only API-level prompt caching). PulSeed's DurableLoop calls the same tools multiple times per iteration (observe → gap → verify), making caching high-value.

**CC Reference**: CC performs API-level prompt caching only. No tool result caching. PulSeed's multi-stage iteration pattern (observe → gap → verify) makes in-iteration caching significantly more valuable than in CC's single-pass model.
...
### 14.4 Relationship to DataSourceAdapter

**Decision** (resolved): Coexist for now. DataSourceAdapters are per-goal configured observation sources with specific query semantics. Tools are general-purpose capabilities. In a future extension, provide `DataSourceToolAdapter` wrapper to unify under ITool interface. No schema changes to existing DataSourceAdapter.
...
### 14.5b Self-Knowledge Tools Relationship

**Decision** (resolved): Self-knowledge tools (`get_goals`, `get_sessions`, `get_trust_state`, etc.) and DurableLoop tools (Glob, Grep, Read, Shell, etc.) are separate layers:
- Self-knowledge: introspection into PulSeed's internal state, used by chat UI via LLM function calling
- DurableLoop tools: perception of external world, used by ObservationEngine/GapCalculator/etc. directly

Future unification path: re-implement self-knowledge tools as ITool instances registered in ToolRegistry with `mode: chat` filter. DurableLoop tools registered with `mode: core` filter. `assembleToolPool()` filters by mode, following CC's `getTools(permissionContext)` pattern.
...
**Rationale**: Keeping the layers separate now avoids refactoring the chat UI and DurableLoop simultaneously. The `mode` filter approach provides a clean unification path without breaking either consumer.
...
### 14.8 Observation Method Evolution
