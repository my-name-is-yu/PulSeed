...
> Implementation alignment: much of this integration has already happened. ChatRunner no longer represents the only tool-using path; the native AgentLoop runtime, DurableLoop phases, and task execution all share the built-in tool substrate. Read this document as a migration/design rationale, not as an untouched future plan.
...
**After foundation track migration (new files in `state/`):**
...
**New tools added in foundation track (migrated from System B):**
...
**foundation track: Integration Migration**
...
- **Integration (DurableLoop)**: DurableLoop with tool layer, full round-trip (area B)
- **Concurrency**: parallel read-only tools execute simultaneously via existing concurrency.ts (area C)
