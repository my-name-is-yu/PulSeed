...
> Implementation alignment: chat/TUI are now built on a stronger native AgentLoop path, and daemon/runtime ownership has evolved since this document was written. Treat `/tend` here as a handoff pattern from bounded chat execution into long-lived goal control, not as a precise wire-level description of the current UI/runtime code.
...
### Default behavior: Core `/tend` command
...
### Extended behavior: TUI integration (future)
