...
Primary map: [Context Sources](./context-sources-map.md).
...
**default behavior scope**: `file` and `http_api` only. `database` and IoT are deferred to a future extension.

**Read-only**: The default behavior performs no write operations. Observation (READ) only.
...
| Type | default behavior support | Description |
|------|-------------|-------------|
| `file` | YES | Reads local files (JSON/CSV/text) |
| `http_api` | YES | Fetches metrics from an external HTTP API via GET/POST |
| `database` | NO (extended behavior) | SQL/NoSQL databases |
| `custom` | NO (extended behavior) | Plugin adapters (IoT, SaaS SDKs, etc.) |
...
## 12. Default behavior and extensions
