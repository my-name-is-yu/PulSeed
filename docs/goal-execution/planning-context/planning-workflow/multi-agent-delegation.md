...
> Design note. Defines how PulSeed delegates a single task across multiple agents with divided responsibilities.
...
> Implementation alignment: current orchestration owners live under `src/orchestrator/` and `src/interface/`, and native `agent_loop` now covers a substantial part of bounded execution. This page describes larger multi-agent/task-group orchestration on top of the DurableLoop + AgentLoop baseline.
...
The `implementor` stage of a Large task is split into two sub-stages:
...
### Default behavior (default behavior): Sequential pipeline + domain-specific observation + persistence + idempotency
...
### Extended behavior: Task decomposition + parallel execution + Plan Gate + strategy feedback
...
### Advanced behavior: Auto pipeline + side-effect detection + contradiction detection + fault tolerance
...
src/orchestrator/execution/
  ...
  task/
    ...
    task-verifier.ts             <- modified (ImpactAnalysis + sycophancy mitigation, advanced behavior)
  adapter-layer.ts               <- modified (capability matching + circuit breaker, advanced behavior)
...
After the pipeline matures, a path is left open in advanced behavior to integrate the existing `task-verifier.ts` L2 through the `reviewer` role.
...
| Role | Responsibility |
|------|----------------|
| `deployer` | Delegate deployment to the appropriate system |
| `monitor` | Continuous monitoring of wearables, DB, APIs, etc. |
| `notifier` | Send notifications to messaging systems |

---

## 10. Fault Tolerance Patterns (advanced behavior)
...
The `PipelineState` snapshot persistence can be extended in the future into a full immutable event log. Each stage's start, completion, and failure would be recorded as events, enabling replay from any point in time. For advanced behavior, `PipelineState` snapshots are sufficient, but the design leaves this extension path open.
