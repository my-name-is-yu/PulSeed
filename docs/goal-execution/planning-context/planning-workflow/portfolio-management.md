...
LLM evaluations are treated as reference information and are not used directly for automated rebalancing decisions (automation is planned for extended behavior).
...
That said, in the default behavior, treating a wait strategy's `allocation` as 0 — allowing all other `active` strategies to use full resources — is also acceptable (see §9).
...
### Default behavior: Sequential Execution + Manual Rebalancing

The default behavior avoids the complexity of parallel execution and is limited to the following:
...
Even in the default behavior, strategies are recorded as explicit entities. Even with sequential execution, a history of "what was tried and what didn't work" accumulates, making the data available when transitioning to extended behavior.

### Extended behavior: Automatic Rebalancing + Parallel Execution
...
### Advanced behavior (Future Vision)
...
## advanced behavior (portfolio-management scope)
...
StrategyDependencyEdge {
  ...
  goal_id: string                 // Only intra-goal dependencies (in advanced behavior)
...
| default behavior is sequential; extended behavior is parallel | Complexity is added incrementally while managing risk |
