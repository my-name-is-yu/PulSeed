...
> Status: Archived or historical design note. This is retained for context and is not public-current guidance.

> Historical architecture draft. For the current production recovery design, see [Runtime Auto-Recovery](../../design/infrastructure/runtime-auto-recovery.md).
...
User-visible workspace artifacts, debug exports, reports, and configuration remain file-backed where they are not internal durable runtime state. The current database-first boundary is maintained in `docs/internal/design/infrastructure/database-first-state-ownership.md`.
