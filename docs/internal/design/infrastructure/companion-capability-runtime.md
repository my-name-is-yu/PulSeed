...
It defines the contract that implementation slices must preserve.
...
| Runtime auth and guardrails | `docs/internal/design/infrastructure/runtime-auth-browser-guardrails.md`, `src/runtime/control/`, `src/runtime/guardrails/` | Auth handoffs, browser sessions, guardrails, and backpressure are hard readiness inputs and fail-closed blockers. |
...
| Runtime control | `docs/internal/design/infrastructure/runtime-control-plane.md`, `src/runtime/control/runtime-control-service.ts`, `src/runtime/store/`, `src/runtime/session-registry/` | Runtime control owns admission, staleness, authority, visibility, control, and audit for action-bearing operations. |
