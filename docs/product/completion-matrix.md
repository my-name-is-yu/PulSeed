...
| durable personal-agent trace spine | Current operating behavior | `tests/contracts/personal-agent-runtime.test.ts`, `src/runtime/personal-agent/*`, and Control DB migration `personal-agent-runtime-state` | Non-trivial production decisions are represented as typed decision trace records before task, run, action, notification, or other materialized effects. |
