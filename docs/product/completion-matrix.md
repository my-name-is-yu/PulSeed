...
| Memory / Soil / Knowledge truth maintenance | Current operating behavior | `src/runtime/store/__tests__/memory-truth-maintenance-store.test.ts`, `tests/replay/memory-truth-maintenance-replay.test.ts`, `tests/product-gauntlet/memory-truth-maintenance-gauntlet.test.ts`, `src/tools/query/MemoryRecallTool/__tests__/MemoryRecallTool.test.ts`, `src/tools/query/KnowledgeQueryTool/__tests__/KnowledgeQueryTool.test.ts`, `src/platform/soil/__tests__/soil-runtime-rebuild-import.test.ts`, `src/interface/cli/commands/__tests__/memory.test.ts`, `src/platform/corrections/__tests__/user-memory-operations.test.ts`, and `src/interface/cli/__tests__/database-first-legacy-store-check.test.ts` | Agent memory, domain knowledge, and shared knowledge production paths use `MemoryTruthMaintenanceStore` typed claims, evidence refs, corrections, forget tombstones, conflicts, recall records, and projection records. StateManager is a root/config/compatibility boundary for these paths. Soil projections and Runtime Event Log summaries consume the typed lifecycle, correction refs retain Event Log/RuntimeGraph linkage after projection and later owner snapshots, post-commit trace failures do not hide committed corrections, production replacement corrections write resolved conflict sets and correction-impact recall records in the same transaction, resolved conflicts restore the selected claim and archive stale losers, operator-restored tombstones allow explicit claim reactivation, domain deletes tombstone typed truth before Soil compatibility records, goal-scoped semantic knowledge queries filter by typed owner scope, runtime rebuild does not reproject inactive truth from stale Soil compatibility records, active-looking entries with inactive correction state are withheld from normal recall/projection, conflicted memory is withheld from normal recall/export, and new raw memory/knowledge `StateManager.writeRaw` production writes fail the DB-first guard. |
...
| runtime event log source-of-truth | Current operating behavior | `tests/contracts/runtime-event-log-source-of-truth.test.ts`, `tests/replay/runtime-event-log-source-of-truth-replay.test.ts`, `npm run test:product-gauntlet`, `src/runtime/store/runtime-event-log.ts`, and Control DB migrations `runtime-event-log-source-of-truth` and `runtime-event-projection-snapshots` | The append-only `runtime_events` table plus RuntimeGraph linkage is the source-of-truth path for major runtime decisions, causal explanation, replay/dedupe evidence, deterministic projection rebuilds, applied projection snapshots, and event-backed restoration of goal/task, interaction-authority, runtime-control, and commitment current-state rows. Runtime-control operation transitions and attention-led commitment lifecycle transitions append typed events before projection writes. Side-effect queues, transport receipts, scheduler rows, memory truth rows, approval waits, tool effects, and live daemon/session status rows remain documented owner or summary-only boundaries. |
