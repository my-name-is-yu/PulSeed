...
Primary map: [Loop State](./loop-state-map.md).
...
> Implementation alignment: TimeHorizonEngine now influences a DurableLoop that may also run bounded agentic phases and native AgentLoop task execution. The temporal model still applies, but the surrounding orchestration is no longer a single flat loop body.
