...
inputs as well as database files. See [Runtime State](../../operating/runtime-operations/runtime-state.md)
...
- [Module Map](./module-layout.md) is the code navigation companion
- `src/` is the implementation truth

Public design contracts now live under named cluster maps such as
`docs/runtime-architecture/`, `docs/goal-execution/`, and
`docs/companion-autonomy/`. Some describe product direction or boundary context
rather than the exact current runtime path.
