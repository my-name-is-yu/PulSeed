...
> Implementation alignment: the risk model remains relevant, but many file paths below predate the current `src/orchestrator/`, `src/platform/`, and `src/base/` layout. Treat implementation-location bullets as approximate unless they already match the current tree.
...
**Rate limit granularity**: The trust rate limit uses a fixed 1-hour window. This may be too short in production, but is kept conservative for default behavior. The window size may be made configurable in the future.
