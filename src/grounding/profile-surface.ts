...
export function buildRelationshipProfileSurfaceProjection(
  ...
  const sourceRefs = input.context.items.map((item) => surfaceSourceForProfileItem(item, input.context));
  const includedSources = input.context.items
    .map((item) => ({ item, source: surfaceSourceForProfileItem(item, input.context) }))
    .filter(({ item, source }) => (
      item.status === "active"
  ...
  const includedContext = includedSources.map(({ item, source }) => ({
    ...
    source_ref: source,
    use_class: "runtime_grounding" as const,
    excerpt: item.value,
  ...
  const excludedContext = excludedSources.map(({ source }) => ({
    ...
    source_ref: source,
    requested_use: "runtime_grounding" as const,
    blocked_by: [surfaceGate(blockedGateForSource(source), "blocked", input.now)],
  ...
  return SurfaceProjectionSchema.parse({
    ...
    allowed_runtime_uses: ["runtime_grounding", "surface_projection"],
    ...
    rationale_entries: [
      ...
      ...excludedSources.map(({ source }) => ({
        ...
        gate: blockedGateForSource(source),
...
export function formatRelationshipProfileSurfaceContext(
  ...
  return [
    `Relationship profile retrieval context Surface (surface_id=${projection.id}; requested_use=${projection.requested_use})`,
...
function surfaceSourceForProfileItem(
  ...
  return {
    ...
    allowed_uses: inScope ? ["runtime_grounding", "surface_projection"] : ["never_use_directly"],
    not_allowed_uses: [
      "side_effect_authorization",
      "stale_session_authorization",
      "raw_prompt_injection",
      ...(inScope ? [] : ["runtime_grounding" as const, "cross_scope_reuse" as const]),
    ],
    lifecycle,
...
function blockedGateForSource(source: SurfaceMemorySourceRef): SurfaceGateKind {
  if (source.lifecycle !== "active" || source.correction_state !== "current") return "lifecycle";
  if (source.not_allowed_uses.includes("cross_scope_reuse") || !source.allowed_uses.includes("runtime_grounding")) return "scope";
