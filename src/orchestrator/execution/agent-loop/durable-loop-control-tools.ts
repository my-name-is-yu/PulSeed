...
export function createDurableLoopControlTools(service: DurableLoopControlToolset): ITool[] {
  const tools: ITool[] = [
    new DurableLoopControlTool("core_goal_status", "Read DurableLoop goal status.", "read_only", (input) => service.goalStatus(input), schemas.core_goal_status),
  ];
  if (service.goalCreate) tools.push(new DurableLoopControlTool("core_goal_create", "Create a DurableLoop goal.", "write_local", (input) => service.goalCreate!(input), schemas.core_goal_create));
  if (service.tendGoal) tools.push(new DurableLoopControlTool("core_tend_goal", "Create a DurableLoop goal and start it in the daemon for long-running background execution.", "write_local", (input) => service.tendGoal!(input), schemas.core_tend_goal));
  if (service.goalStart) tools.push(new DurableLoopControlTool("core_goal_start", "Start or resume a DurableLoop goal in the daemon.", "write_local", (input) => service.goalStart!(input), schemas.core_goal_start));
  if (service.goalPause) tools.push(new DurableLoopControlTool("core_goal_pause", "Pause a DurableLoop goal.", "write_local", (input) => service.goalPause!(input), schemas.core_goal_pause));
  if (service.goalResume) tools.push(new DurableLoopControlTool("core_goal_resume", "Start or resume a DurableLoop goal in the daemon.", "write_local", (input) => service.goalResume!(input), schemas.core_goal_resume));
  if (service.goalCancel) tools.push(new DurableLoopControlTool("core_goal_cancel", "Cancel a DurableLoop goal.", "write_local", (input) => service.goalCancel!(input), schemas.core_goal_cancel));
  if (service.taskStatus) tools.push(new DurableLoopControlTool("core_task_status", "Read DurableLoop task status.", "read_only", (input) => service.taskStatus!(input), schemas.core_task_status));
  if (service.taskPrioritize) tools.push(new DurableLoopControlTool("core_task_prioritize", "Set DurableLoop task priority.", "write_local", (input) => service.taskPrioritize!(input), schemas.core_task_prioritize));
  if (service.runCycle) tools.push(new DurableLoopControlTool("core_run_cycle", "Run one bounded DurableLoop cycle.", "write_local", (input) => service.runCycle!(input), schemas.core_run_cycle));
  return tools;
}
