...
import {
  ...
  type KnowledgeTransferStateStorePort,
} from "../../../runtime/store/knowledge-transfer-state-store.js";
...
export class KnowledgeTransfer {
  ...
  private readonly transferStateStore: KnowledgeTransferStateStorePort;
  ...
  constructor(deps: {
    ...
    knowledgeTransferStore?: KnowledgeTransferStateStorePort;
    gateway?: IPromptGateway;
  }) {
    this.transferTrust =
      deps.transferTrust ??
      new TransferTrustManager({ stateManager: deps.stateManager });
    this.transferStateStore =
      deps.knowledgeTransferStore ??
      new KnowledgeTransferStateStore(deps.stateManager.getBaseDir());
  ...
  private async loadSnapshot(): Promise<void> {
    try {
      const snapshot = await this.transferStateStore.loadSnapshot();
      if (snapshot) {
  ...
  private async persistSnapshot(): Promise<void> {
    ...
    const snapshot = buildKnowledgeTransferSnapshot({
      transfers: Array.from(this.candidates.values()),
      results: Array.from(this.results.values()),
      effectivenessRecords: getEffectivenessRecords(this.effectivenessRecords),
      applyContexts: Object.fromEntries(this.applyContexts.entries()),
      patternTrackers: Object.fromEntries(this.patternTrackers.entries()),
      crossGoalPatterns: Array.from(this.crossGoalPatterns.values()),
    });
    try {
      await this.transferStateStore.saveSnapshot(snapshot);
    } catch {
  ...
  private async _loadLastAggregatedAt(): Promise<string | null> {
    ...
    try {
      this.lastAggregatedAt = await this.transferStateStore.loadLastAggregatedAt();
  ...
  private async _saveLastAggregatedAt(ts: string): Promise<void> {
    try {
      await this.transferStateStore.saveLastAggregatedAt(ts);
    } catch {
      // non-fatal
    }
