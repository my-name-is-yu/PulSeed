...
export class MemoryTruthMaintenanceStore {
  ...
  async saveOwnerSnapshot(input: MemoryTruthSnapshotInput): Promise<void> {
    ...
    db.transaction((sqlite) => {
      const autoGeneratedTombstones: ForgetTombstone[] = [];
      ...
      for (const existing of existingClaims) {
        if (!nextClaimIds.has(existing.claim_id) && existing.lifecycle !== "forgotten") {
          ...
          autoGeneratedTombstones.push(tombstone);
      ...
      if (input.emitRuntimeEvent ?? this.options.appendRuntimeEvents) {
        const eventClaimIds = unique([
          ...acceptedClaims.map((claim) => claim.claim_id),
          ...autoGeneratedTombstones.map((tombstone) => tombstone.claim_id),
        ]);
        const eventTombstoneIds = unique([
          ...acceptedTombstones.map((tombstone) => tombstone.tombstone_id),
          ...autoGeneratedTombstones.map((tombstone) => tombstone.tombstone_id),
        ]);
        appendRuntimeEventEnvelopeInTransaction(sqlite, this.memoryTruthRuntimeEventInput({
          ...
          idempotencyKey: `memory-truth:snapshot:${ownerKind}:${ownerScope}:${stableId([
            ...acceptedClaims.map((claim) => `${claim.claim_id}:${claim.updated_at}:${claim.lifecycle}`),
            ...autoGeneratedTombstones.map((tombstone) => `${tombstone.claim_id}:${tombstone.created_at}:forgotten`),
          ].join("|"))}`,
          sourceRef: { kind: "memory_truth_owner", ref: `${ownerKind}:${ownerScope}` },
          targetRefs: eventClaimIds.map((claimId) => ({ kind: "memory_claim", ref: claimId })),
          payload: {
            schema_version: "runtime-event-payload/memory-truth-maintenance/v1",
            operation: "snapshot",
            correction_ref: null,
            claim_ids: eventClaimIds,
            projection_ids: projections.map((projection) => projection.projection_id),
            recall_ids: [],
            tombstone_ids: eventTombstoneIds,
  ...
  async listCorrections(input: string | ListCorrectionsInput = {}): Promise<CorrectionRef[]> {
    const filters = typeof input === "string" ? { targetClaimId: input } : input;
    const db = await this.database();
    return db.read((sqlite) => {
      const clauses: string[] = [];
      const params: unknown[] = [];
      if (filters.targetClaimId) {
        clauses.push("refs.target_claim_id = ?");
        params.push(filters.targetClaimId);
      }
      if (filters.ownerKind) {
        clauses.push("claims.owner_kind = ?");
        params.push(filters.ownerKind);
      }
      if (filters.ownerScope) {
        clauses.push("claims.owner_scope = ?");
        params.push(filters.ownerScope);
      }
      const where = clauses.length > 0 ? `WHERE ${clauses.join(" AND ")}` : "";
      const rows = sqlite.prepare(`
        SELECT refs.correction_json
        FROM memory_correction_refs refs
        JOIN memory_claims claims ON claims.claim_id = refs.target_claim_id
        ${where}
        ORDER BY refs.created_at, refs.correction_id
      `).all(...params);
