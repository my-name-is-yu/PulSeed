...
- Removed normal runtime references to `run-specs/<id>.json`; legacy files are ignored by the normal store and read only by explicit doctor/repair import.
...
- Fallback review found the legacy import boundary was missing for valid legacy RunSpec files.
- Added `importLegacyRunSpecState`, wired it into `doctor --repair`, and recorded imported/blocked legacy inputs in `control_legacy_imports`.
