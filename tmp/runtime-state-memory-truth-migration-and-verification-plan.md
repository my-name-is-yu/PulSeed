...
- Fresh audit blocker: correction commits could be followed by a truth snapshot from Soil projection, overwriting `runtime_event_ref` and `runtime_graph_refs` on `CorrectionRef`. Fix: correction hooks call `projectAgentMemory(..., { persistTruth: false })`, keeping Soil projection writes after the transaction while preventing a second truth snapshot from erasing event/graph refs. `user-memory-operations.test.ts` and `knowledge-manager-lint.test.ts` assert the real user and dream_lint caller paths retain those refs.
