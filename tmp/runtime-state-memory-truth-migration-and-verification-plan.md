...
- Fresh audit blocker: goal-scoped `KnowledgeQueryTool` semantic requests used global vector hits and relabeled returned entries with the requested `goalId`. Fix: `KnowledgeManager.searchKnowledge` accepts a goal scope, searches enough vector candidates to filter by `metadata.goal_id`, and then loads typed domain knowledge for that owner scope. `KnowledgeQueryTool.test.ts` uses the real KnowledgeManager and typed store path to prove a higher-ranked other-goal vector hit is excluded from a scoped semantic query.
...
- GitHub Codex review blocker: agent-memory correction returned an error when post-commit trace persistence failed, even though the truth transaction had already committed. Fix: the agent-memory user operation records the trace best-effort after the typed transaction and returns the committed correction; `user-memory-operations.test.ts` injects a trace write failure and proves stale memory remains withheld while the replacement and correction row persist.
- GitHub Codex review blocker: resolved conflict sets left claims in `lifecycle=conflicted`. Fix: `MemoryTruthMaintenanceStore` records pre-conflict lifecycle metadata, restores the resolved winner to its previous normal projection eligibility, and archives losing claims; `memory-truth-maintenance-store.test.ts` proves the transition.
- GitHub Codex review blocker: domain knowledge deletion tombstoned Soil compatibility records without tombstoning typed truth first. Fix: `KnowledgeMemoryStateStore.deleteDomainKnowledge` now writes an empty typed domain owner snapshot before Soil tombstones; `knowledge-memory-state-store.test.ts` proves `KnowledgeManager.loadKnowledge` returns no active entries and typed inactive claim state remains inspectable.
- GitHub Codex review blocker: normal agent-memory save wrote truth once and then Soil projection could write the same truth snapshot again. Fix: `saveAgentMemoryStore` saves the typed owner snapshot once, then projects Soil with `persistTruth=false`; `knowledge-memory-state-store.test.ts` asserts a production `KnowledgeManager.saveAgentMemory` caller invokes `saveOwnerSnapshot` once.
- GitHub Codex review blocker: inactive Soil correction audit records were hidden from the explicit Soil compatibility fallback. Fix: `KnowledgeMemoryStateStore.loadAgentMemoryStore` reads agent-memory entries and correction records with `active_only=false`; `knowledge-memory-state-store.test.ts` proves correction history survives a Soil-only compatibility fallback while remaining inactive for normal search.
- GitHub Codex review blocker: forget tombstones blocked reactivation even after operator restore. Fix: resurrection blocking now applies only to tombstones with `prevents_reimport=true` and `operator_restored_at=null`; `memory-truth-maintenance-store.test.ts` proves a restored tombstone permits explicit active claim reactivation.
- Fresh review blocker: docs claimed production correction transactions persisted conflict and recall records, but the production `commitAgentMemoryCorrectionToTruth` caller supplied only correction/replacement/tombstone/projection records. Fix: replacement corrections now add a resolved conflict set and every agent-memory correction/forget/retract adds a correction-impact graph recall record inside `applyCorrectionTransaction`; `user-memory-operations.test.ts` and `memory-truth-maintenance-replay.test.ts` prove the production caller path and restart path.
