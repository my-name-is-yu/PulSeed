...
- Post-correction Soil projection uses `persistTruth=false`, so projection writes cannot overwrite `CorrectionRef.runtime_event_ref` or `runtime_graph_refs` after the typed correction transaction commits.
