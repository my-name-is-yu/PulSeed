...
| domain/shared knowledge load fallback | `KnowledgeManager.loadKnowledge` and `querySharedKnowledge` use typed truth presence before Soil compatibility fallback | `knowledge-memory-state-store.test.ts` proves inactive typed truth blocks stale Soil record resurrection |
