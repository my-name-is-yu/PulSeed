...
| production correction conflict/recall records | `commitAgentMemoryCorrectionToTruth` supplies a correction-impact graph `RecallRecord` for every agent-memory correction/forget/retract and a resolved `ConflictSet` for replacement corrections | `user-memory-operations.test.ts` proves the real user correction caller path writes these rows; `memory-truth-maintenance-replay.test.ts` proves they survive restart |
...
| post-commit correction trace | `runUserMemoryOperation` commits the typed agent-memory correction transaction before recording PersonalAgent/authority trace evidence; trace persistence is best-effort after commit | `user-memory-operations.test.ts` injects `PersonalAgentRuntimeStore.recordTrace` failure and proves the committed correction/replacement remains returned and persisted |
| knowledge query semantic fallback and scope | `KnowledgeQueryTool.call` -> semantic search availability and owner-scope contract | `KnowledgeQueryTool.test.ts` proves unavailable semantic index reports `semantic_unavailable`, available-empty semantic search does not call keyword fallback, and goal-scoped semantic search excludes higher-ranked other-goal vector hits through the real KnowledgeManager typed-store path |
| domain knowledge delete | `KnowledgeMemoryStateStore.deleteDomainKnowledge` writes an empty typed domain owner snapshot before Soil compatibility tombstones | `knowledge-memory-state-store.test.ts` proves `KnowledgeManager.loadKnowledge` returns no active entries and typed inactive claim state remains inspectable |
| conflict resolution | `MemoryTruthMaintenanceStore.upsertConflict` restores the resolved winner and archives losing claims instead of leaving both claims conflicted | `memory-truth-maintenance-store.test.ts` proves `conflict_set.status=resolved` clears the winner lifecycle and keeps the stale loser out of normal projection |
| Soil compatibility correction history | `KnowledgeMemoryStateStore.loadAgentMemoryStore` reads inactive agent-memory correction audit rows only for explicit compatibility fallback | `knowledge-memory-state-store.test.ts` proves Soil-only correction history remains loadable without making correction audit rows normal-search active |
| operator-restored tombstone | `MemoryTruthMaintenanceStore.saveOwnerSnapshot` blocks stale reimport only for unrestored tombstones with `prevents_reimport=true` | `memory-truth-maintenance-store.test.ts` proves operator restoration allows explicit claim reactivation |
...
- Caller-path test: goal-scoped knowledge semantic query keeps vector hits inside the requested typed owner scope instead of relabeling cross-goal results. Added in `KnowledgeQueryTool.test.ts` with real KnowledgeManager and typed domain-knowledge store calls.
...
- Caller-path test: user and dream_lint correction paths retain `runtime_event_ref` and `runtime_graph_refs` after post-transaction Soil projection, and a later normal `KnowledgeManager.saveAgentMemory` snapshot does not erase the refs. Added in `user-memory-operations.test.ts` and `knowledge-manager-lint.test.ts`.
- Caller-path test: user replacement correction writes a resolved conflict set and graph recall record in the same production transaction. Added in `user-memory-operations.test.ts`.
- Caller-path test: user agent-memory correction returns the committed correction when post-commit trace persistence fails. Added in `user-memory-operations.test.ts`.
- Caller-path test: `KnowledgeManager.saveAgentMemory` writes the typed truth owner snapshot once and projects Soil with `persistTruth=false`. Added in `knowledge-memory-state-store.test.ts`.
- Caller-path test: domain knowledge delete tombstones typed truth before Soil compatibility records, so normal `KnowledgeManager.loadKnowledge` cannot fall back to stale records. Added in `knowledge-memory-state-store.test.ts`.
- Compatibility caller test: Soil-only fallback reads inactive correction audit rows for history/dedupe context without exposing them as active normal-search records. Added in `knowledge-memory-state-store.test.ts`.
- Contract test: resolving a conflict restores the winner lifecycle/projection eligibility and archives losing claims. Added in `memory-truth-maintenance-store.test.ts`.
- Contract test: operator-restored tombstones no longer block explicit active claim reactivation. Added in `memory-truth-maintenance-store.test.ts`.
